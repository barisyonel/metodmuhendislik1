...
export async function generateMetadata({
  ...
  const { location, locationName, serviceType } = parseSlug(resolvedParams.slug);
  // Eğer locationName boşsa, default metadata döndür
  if (!locationName || !location) {
    return {
      title: "Blog Sayfası Bulunamadı | Metod Mühendislik",
      description: "Aradığınız blog sayfası bulunamadı.",
    };
  }
...
function parseSlug(slug: string | undefined): {
  ...
  // Slug undefined veya boşsa default değerler döndür
  ...
  // En az 2 parça olmalı (location + serviceType)
  if (parts.length < 2) {
    return {
      location: slug,
      locationName: slug,
...
export default async function BlogPostPage({
  ...
  const resolvedParams = params instanceof Promise ? await params : params;
  const { location, locationName, serviceType } = parseSlug(resolvedParams.slug);
  // Eğer locationName boşsa, generateBlogContent çağrılmadan önce kontrol et
  if (!locationName || !location) {
    return (
      <>
        <Header />
        <main className="bg-white min-h-screen pt-24 md:pt-28">
          <div className="container mx-auto px-6 py-12">
            ...
            <p className="text-slate-700">
              Aradığınız blog sayfası bulunamadı.
            </p>
          </div>
        </main>
        <Footer />
          ...
          <div className="container mx-auto px-6 max-w-4xl">
            ...
            <div className="mt-12 p-8 bg-gradient-to-br from-blue-50 to-slate-50 rounded-2xl border-2 border-blue-200">
              ...
              <p className="text-slate-700 mb-6">
                {post.location} bölgesinde elektrik pano ve kaynak hizmetleri
                için uzman ekibimizle iletişime geçin. Ücretsiz keşif ve teklif
                hizmeti sunuyoruz.
              </p>
