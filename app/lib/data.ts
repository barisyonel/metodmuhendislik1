...
function fixTurkishEncoding(text: string | null | undefined): string {
  ...
  try {
    ...
    try {
      ...
      if (turkishCharsInDecoded && !turkishCharsInOriginal && decoded !== text) {
        return decoded;
      }
    } catch {
...
function fixIconEncoding(icon: string | null | undefined, serviceName: string = ""): string {
  ...
  if (icon && icon.trim() !== "") {
    ...
    const validEmojis = /[⚡🔧🔥🎨📦🏗️🔺📐🏭🔌💡⚙️🛠️📏🔨🎯✨]/.test(trimmedIcon);

    // Bozuk encoding karakterleri kontrol et
    const brokenChars = ['â', 'ši', 'Δ', 'Ϋ', 'Ÿ', 'Ž', 'ї', 'Ö', '¥', '◆', '!', 'S'];
    const hasBrokenEncoding = brokenChars.some(char => trimmedIcon.includes(char));
    ...
    if (validEmojis && !hasBrokenEncoding && trimmedIcon.length <= 3 && trimmedIcon === nameBasedIcon) {
      return trimmedIcon;
    }
  }
...
function getIconByServiceName(serviceName: string): string {
  ...
  if (name.includes("elektrik") || name.includes("pano") || name.includes("marin")) return "⚡";
  if (name.includes("lazer") || name.includes("kesim") || name.includes("cnc lazer")) return "🔺";
  if (name.includes("büküm") || name.includes("bukum") || name.includes("cnc büküm") || name.includes("cnc bukum")) return "📐";
  if (name.includes("kaynak") || name.includes("welding") || name.includes("metal kaynak")) return "🔥";
  if (name.includes("boya") || name.includes("toz") || name.includes("elektrostatik")) return "🎨";
  if (name.includes("raf") || name.includes("mağaza") || name.includes("magaza") || name.includes("raf sistemleri")) return "📦";
  if (name.includes("konstrüksiyon") || name.includes("konstruksiyon") || name.includes("çelik") || name.includes("celik") || name.includes("steel")) return "🏗️";
...
export async function getServices(): Promise<Service[]> {
  try {
    ...
    return servicesData.map(service => {
      ...
      const fixedDescription = originalDescription ? fixTurkishEncoding(originalDescription) : undefined;
      ...
      if (process.env.NODE_ENV === 'development') {
        if (originalIcon !== fixedIcon) {
          console.log(`🎨 Icon düzeltmesi: "${fixedName}" -> "${originalIcon}" -> "${fixedIcon}"`);
        } else {
      ...
      if (normalizedHref.includes('üretimi') || normalizedHref.includes('uretimi')) {
        normalizedHref = normalizedHref.replace(/[üÜ]/g, 'u').replace(/[çÇ]/g, 'c').replace(/[ğĞ]/g, 'g').replace(/[şŞ]/g, 's').replace(/[ıİ]/g, 'i').replace(/[öÖ]/g, 'o');
      }
      ...
      if (/[üçğışöÜÇĞIŞÖ]/.test(normalizedHref)) {
        const hrefMapping: { [key: string]: string } = {
          '/hizmetler/elektrik-pano-üretimi': '/hizmetler/elektrik-pano-uretime',
          '/hizmetler/elektrik-pano-uretimi': '/hizmetler/elektrik-pano-uretime',
          '/hizmetler/cnc-büküm': '/hizmetler/cnc-bukum',
          '/hizmetler/mağaza-raf-ve-ürünleri': '/hizmetler/magaza-raf-ve-urunleri',
          '/hizmetler/çelik-konstrüksiyon': '/hizmetler/celik-konstruksiyon',
        };
        normalizedHref = hrefMapping[normalizedHref] || normalizedHref.replace(/[üçğışöÜÇĞIŞÖ]/g, (char) => {
          const map: { [key: string]: string } = { 'ü': 'u', 'Ü': 'U', 'ç': 'c', 'Ç': 'C', 'ğ': 'g', 'Ğ': 'G', 'ş': 's', 'Ş': 'S', 'ı': 'i', 'İ': 'I', 'ö': 'o', 'Ö': 'O' };
      ...
      if (process.env.NODE_ENV === 'development') {
        if (originalName !== fixedName) {
          console.log(`✅ Name encoding düzeltmesi: "${originalName}" -> "${fixedName}"`);
        }
        if (originalDescription && originalDescription !== fixedDescription) {
          console.log(`✅ Description encoding düzeltmesi: "${originalDescription}" -> "${fixedDescription}"`);
        }
        if (service.href !== normalizedHref) {
          console.log(`✅ Href normalize düzeltmesi: "${service.href}" -> "${normalizedHref}"`);
        }
      }
