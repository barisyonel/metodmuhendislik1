...
export default async function ProductDetail({
  ...
  if (product.images) {
    try {
      const imagesString = typeof product.images === 'string' ? product.images : String(product.images);
  ...
  if (product.image) {
    ...
    if (imageUrl) {
      if (!productImages.includes(imageUrl)) {
        ...
        productImages = [imageUrl, ...productImages.filter(img => img !== imageUrl)];
      }
    }
  }
        ...
        <section className="relative pt-24 md:pt-32 pb-12 md:pb-16 overflow-hidden">
          ...
          <div className="container mx-auto px-6 relative z-10">
            ...
            <nav className="flex items-center gap-2 text-sm mb-8 animate-slide-up">
              ...
              <Link href="/urunler" className="text-slate-500 hover:text-blue-600 transition-colors">
                Ürünler
              </Link>
              <span className="text-slate-300">/</span>
              {product.category && (
              ...
              <div className="bg-gradient-to-br from-blue-50 to-slate-50 rounded-3xl p-8 shadow-xl border border-blue-100">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between py-3 border-b border-blue-100">
                    ...
                    <span className="text-slate-900 font-bold">{product.category || "Belirtilmemiş"}</span>
                  </div>
                  <div className="flex items-center justify-between py-3 border-b border-blue-100">
                    <span className="text-slate-600 font-semibold flex items-center gap-2">
                      <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className="text-slate-900 font-bold">#{product.id}</span>
                  </div>
              ...
              <div className="prose prose-lg prose-slate max-w-none">
                ...
                <div className="bg-blue-50 border-l-4 border-blue-600 p-6 rounded-r-xl">
                  <p className="text-slate-700 leading-relaxed">
                    Metod Mühendislik olarak, <strong className="text-slate-900">{product.title}</strong> konusunda uzman ekibimizle
                    hizmet vermekteyiz. <strong className="text-slate-900">{product.category}</strong> kategorisindeki bu ürünümüz,
